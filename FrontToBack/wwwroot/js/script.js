$(document).ready(function () {
    ...
    $(document).on("keyup", "#input-search", function () {
        ...
        $("#searchproductlis li").slice(1).remove();
        if (search.length > 0) {
            $.ajax({
                url: 'Product/Search?search=' + search,
                type: 'Get',
                success: function (res) {
                    $("#searchproductlis").append(res);
                }
            })
        }
    })
    ...
    $(document).on('click', '#loadmore', function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                //    let a = $("<a>");
                //    let img = $("<img>").attr("src", "/img/" + item.image).addClass("img-fluid");
                //    a.append(img);
                //    divimg.append(a);
                ...
                //    let productitem = $("<div>").addClass("product-item text-center").attr("data-id", item.category.name);
                //    productitem.append(divimg, divtitle, divprice);

                //    let divCol = $("<div>").addClass("col-sm-6 col-md-4 col-lg-3 mt-3");
                //    divCol.append(productitem);
